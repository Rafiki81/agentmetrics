...
type GitMonitor struct {
	// Cache last known state per directory to detect changes
...
func (gm *GitMonitor) Collect(a *agent.AgentInstance) {
	if a.WorkDir == "" {
		return
	}
...
// gitDiffStats returns lines added, removed, and files changed
// Combines staged + unstaged diffs
...
func parseDiffStat(dir string, args ...string) (added, removed, files int) {
	...
	// Use numstat for precise counts
	...
	if err != nil {
		// Fallback to counting stat output lines
		lines := strings.Split(strings.TrimSpace(string(out)), "\n")
		return 0, 0, len(lines) - 1 // last line is summary
	}
