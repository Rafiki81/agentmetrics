...
func (nm *NetworkMonitor) GetConnections(pid int) []agent.NetConnection {
	// Use lsof to find network connections for the process
...
// parseLsofNetLine parses a single lsof -i output line
func parseLsofNetLine(line string) *agent.NetConnection {
	...
	// NODE (fields[7]) must be TCP or UDP — skip anything else
	...
	// Validate: network names contain ":" (e.g., "localhost:8080" or "*:3000")
...
// GetListeningPorts returns ports that are being listened on
func (nm *NetworkMonitor) GetListeningPorts() map[int]int {
	// Returns map of port -> PID
	...
	for i, line := range lines {
		...
		// Extract port from NAME field (e.g., "*:8080" or "localhost:3000")
		name := fields[8]
