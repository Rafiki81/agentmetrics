...
type LocalModelMonitor struct {
	...
	// Track previous request counts for rate calculation
...
// serverDef defines a local model server to probe
type serverDef struct {
	Name         string
	ID           string
	DefaultPort  int
	ProcessNames []string
	ProbeFunc    func(lm *LocalModelMonitor, endpoint string) (*agent.LocalModelInfo, error)
}

// knownServers returns the built-in server definitions
func knownServers() []serverDef {
	return []serverDef{
		...
		},
		{
			...
			ID:           "lm-studio",
			DefaultPort:  1234,
			ProcessNames: []string{"lms", "LM Studio", "lmstudio"},
		},
		{
			Name:         "llama.cpp",
			ID:           "llama-cpp",
			DefaultPort:  8080,
			ProcessNames: []string{"llama-server", "llama-cli", "server"},
		},
		{
			Name:         "vLLM",
			ID:           "vllm",
		...
		},
		{
			...
			ID:           "text-gen-webui",
			DefaultPort:  5000,
			ProcessNames: []string{"text-generation"},
		},
		{
			Name:         "GPT4All",
			ID:           "gpt4all",
			DefaultPort:  4891,
			ProcessNames: []string{"gpt4all", "chat"},
		},
	}
}
...
func (lm *LocalModelMonitor) Collect() []agent.LocalModelInfo {
	...
	// Check configured custom endpoints first
	...
	// Check known servers on their default ports
	for _, srv := range knownServers() {
		// Skip if already found via custom endpoints
		alreadyFound := false
...
// ollamaPSResponse represents Ollama's /api/ps response (running models)
		...
		if psResp.StatusCode == http.StatusOK {
			...
			if json.Unmarshal(psBody, &ps) == nil {
				for _, running := range ps.Models {
					...
					// Mark the model as running in the list
...
// --- OpenAI-compatible probing (LM Studio, vLLM, LocalAI, llama.cpp) ---
...
// findProcessPID tries to find the PID for a process by name
...
// getProcessStats gets CPU and memory for a PID
...
// calculateRates computes tokens/sec and request rates
