	...
	return &Registry{
		Agents: []AgentInfo{
			...
			},
			{
				...
				LogPaths: []string{
					filepath.Join(home, ".windsurf", "logs"),
				},
				Ports:       []int{},
...
func (r *Registry) FindByProcess(processName string) *AgentInfo {
	for i, agent := range r.Agents {
		for _, pname := range agent.ProcessNames {
...
func (r *Registry) FindByCmdLine(cmdline string) *AgentInfo {
	for i, agent := range r.Agents {
		for _, pattern := range agent.DetectPatterns {
...
func containsWord(s, word string) bool {
	// Simple substring match - could be improved with word boundary detection
